export const Marine = () => (
  <svg
    version="1.1"
    id="Layer_1"
    xmlns="http://www.w3.org/2000/svg"
    xmlns:xlink="http://www.w3.org/1999/xlink"
    viewBox="0 0 504 504"
    xml:space="preserve"
    width="30px"
    height="30px"
    fill="#000000"
  >
    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
    <g
      id="SVGRepo_tracerCarrier"
      stroke-linecap="round"
      stroke-linejoin="round"
    ></g>
    <g id="SVGRepo_iconCarrier">
      {" "}
      <circle style="fill:#324A5E;" cx="252" cy="252" r="252"></circle>{" "}
      <polygon
        style="fill:#FF7058;"
        points="380.2,339.7 123.8,339.7 96.9,257.4 407.1,257.4 "
      ></polygon>{" "}
      <path
        style="fill:#84DBFF;"
        d="M374.6,311.3c-19.1,0-35.2,12.4-40.9,29.5c-5.7-17.2-21.9-29.5-40.9-29.5 c-19.1,0-35.2,12.4-40.9,29.5c-5.7-17.2-21.9-29.5-40.9-29.5c-18.9,0-34.9,12.1-40.7,29c-5.9-16.9-21.9-29-40.7-29 c-23.8,0-43.2,19.3-43.2,43.2c0,4.9,0.9,9.6,2.4,14c5.8-17,21.9-29.2,40.8-29.2c18.3,0,33.9,11.4,40.2,27.6c0.2,0.5,0.3,1.1,0.5,1.6 c0-0.1,0-0.1,0.1-0.2c0,0.1,0.1,0.1,0.1,0.2c0.2-0.5,0.3-1.1,0.5-1.6c6.3-16.1,21.9-27.6,40.2-27.6c18.9,0,35,12.2,40.8,29.2 c0-0.1,0.1-0.3,0.1-0.4c0,0.1,0.1,0.3,0.1,0.4c5.8-17,21.9-29.2,40.8-29.2s35,12.2,40.8,29.2c0-0.1,0.1-0.3,0.1-0.4 c0,0.1,0.1,0.3,0.1,0.4c5.8-17,21.9-29.2,40.8-29.2s35,12.2,40.8,29.2c1.5-4.4,2.4-9.1,2.4-14C417.8,330.6,398.5,311.3,374.6,311.3z "
      ></path>{" "}
      <rect
        x="131.1"
        y="183.6"
        style="fill:#FFD05B;"
        width="168.5"
        height="73.7"
      ></rect>{" "}
      <g>
        {" "}
        <circle
          style="fill:#4CDBC4;"
          cx="175.8"
          cy="220.5"
          r="11"
        ></circle>{" "}
        <circle style="fill:#4CDBC4;" cx="215.4" cy="220.5" r="11"></circle>{" "}
        <circle style="fill:#4CDBC4;" cx="255" cy="220.5" r="11"></circle>{" "}
      </g>{" "}
      <rect
        x="161"
        y="123.4"
        style="fill:#FFFFFF;"
        width="33.1"
        height="60.1"
      ></rect>{" "}
      <rect
        x="161"
        y="123.4"
        style="fill:#CED5E0;"
        width="33.1"
        height="12.1"
      ></rect>{" "}
      <path
        style="fill:#E6E9EE;"
        d="M188.1,106.1c1.6-4.2,3.6-8.7,7.7-10.5c4.5-1.9,10.2,0.4,14.5-1.9c2.6-1.4,4.3-4.2,6.9-5.4 c4-1.8,9,0.5,13.1-1.2c2.6-1.1,4.4-3.6,6.7-5.3c5-3.5,12.1-2.4,17.1,1.2c1.2,0.9,3.4,3.4,4.1,7.5c0.7,4.2,0,8.8-2.4,12.3 c-2.5,3.7-7,5.8-11.4,6.2c-4.5,0.4-8.9-0.7-13.1-2.3c-2.8-1.1-5.5-2.4-8.5-2.2c-2.2,0.1-4.4,1-6.6,1.4c-7.5,1.5-15.9-2.1-22.7,1.4 c-2,1-3.6,2.6-5.6,3.4C186.9,109.4,187.5,107.6,188.1,106.1z"
      ></path>{" "}
    </g>
  </svg>
);

export default Marine;
